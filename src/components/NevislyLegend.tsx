...
export default function NevislyLegend() {
  ...
  return <div className="relative">
    ...
    {open&&<div className="absolute right-0 top-10 z-50 max-h-[80vh] w-80 max-w-[90vw] space-y-4 overflow-auto rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs text-zinc-400 shadow-xl">
      ...
      <p><strong className="text-zinc-200">Schedule</strong>: regular-season off-night access and Yahoo Weeks 24–26 are modeled separately with exact dates. Nevisly allocates the projected roster into C2/LW2/RW2/D4 on each non-playoff off-night date, so an off-night game only helps when it is actually usable. Playoff dates use their own exact lineup simulation and are excluded from the season term to avoid double counting. Missing exact off-night dates are neutral, and schedule never changes intrinsic Player Value.</p>
