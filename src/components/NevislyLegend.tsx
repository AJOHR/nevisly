...
export default function NevislyLegend() {
  ...
  return <div className="relative">
    ...
    {open&&<div className="absolute right-0 top-10 z-50 max-h-[80vh] w-80 max-w-[90vw] space-y-4 overflow-auto rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs text-zinc-400 shadow-xl">
      ...
      <p><strong className="text-emerald-400">Draft Urgency</strong> uses a cached Yahoo standard-scoring ADP order for opponent demand, including market-listed goalies and unprojected players. Only on the clock, its separate adjustment compares taking this player now plus a feasible next-pick starter upgrade. SAFE/POSSIBLE/RISKY mean outside/near/inside the market-order removal window, not survival probabilities. Missing or ambiguous ADP is UNKNOWN and is not assumed available as a future target. With no matched future targets, timing is neutral. Player Value and Team Fit stay unchanged.</p>
