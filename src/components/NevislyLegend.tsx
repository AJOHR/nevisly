...
export default function NevislyLegend() {
  ...
  return <div className="relative">
    ...
    {open&&<div className="absolute right-0 top-10 z-50 max-h-[80vh] w-80 max-w-[90vw] space-y-4 overflow-auto rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs text-zinc-400 shadow-xl">
      ...
      <p><strong className="text-emerald-400">Draft Urgency</strong> uses cached Yahoo ADP for opponent demand and bounded paths through the next two own selections. It also measures near-term tier scarcity: how much intrinsic value is lost at a player's eligible position(s) if you wait one turn. This uses the best same-position player actually modeled to survive, never the eventual D40-style fringe. SAFE/POSSIBLE/RISKY still describe the market removal window, not survival probabilities.</p>
      ...
      <p><strong className="text-zinc-200">Schedule</strong>: the regular season and Yahoo Weeks 24–26 are modeled separately with exact dates. For the regular season, Nevisly allocates the projected roster on every non-playoff NHL game date and measures only production lost to daily-lineup congestion. Off nights help by preventing lost starts; they never create extra production on top of the season projection. Playoff dates use their own exact lineup simulation. Missing exact regular-season dates are neutral, and schedule never changes intrinsic Player Value.</p>
