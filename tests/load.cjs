...
exports.actions = initial => {
 ...
 const names=['draftPlayer','undoDraftPlayer','undoLastPick','getSnakeTeamIdForPick'];const found=[];
 ...
 const deps={draftedIds:new Set(initial.map(p=>p.playerId)),leagueTeams:12,setDraftPicks:f=>picks=f(picks),setSelectedDraftTeamId:id=>selected=id};
 ...
 return {...new Function(...Object.keys(deps),js+';return {draftPlayer,undoDraftPlayer,undoLastPick};')(...Object.values(deps)),get picks(){return picks;},get selected(){return selected;}};
};
